...
const { manShoe, isLoading, isError } = useManShoe(itemId)
</script>

<template>
	<div>
		<div v-if="isLoading"></div>
		<section v-if="manShoe" class="w-full animation p-5 ">
			<div class="border h-screen p-5">
				<h1 class="text-center font-bold mb-10 border-b pb-5 text-[1.8rem]">{{ manShoe?.name }}</h1>
				<div class="w-full flex gap-5 item-center border-b pb-10">
					<div
		...
		<div v-if="isError">Something has went wrong</div>
	</div>
</template>
